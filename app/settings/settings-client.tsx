...
export function SettingsClientPage({ initialSettings }: SettingsClientPageProps) {
  ...
  // File Upload handler with auto canvas compression (keeps size < 10KB)
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setErrorMessage("로고 파일은 이미지 양식(PNG, JPG, SVG)이어야 합니다.");
      return;
    }
      ...
      if (event.target?.result && typeof event.target.result === "string") {
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            setSchoolLogo(rawData);
          }
        };
        img.onerror = () => setSchoolLogo(rawData);
        img.src = rawData;
      }
    };
